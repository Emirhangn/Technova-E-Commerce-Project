...
const AdminPanel = () => {
    ...
    const [formData, setFormData] = useState({
        name: "", price: "", image: "", brand: "", rating: 5.0, reviews: ""
    });
...
const addMutation = useMutation({
    mutationFn: (newProduct) => axios.post('http://localhost:8080/api/products', newProduct),
    onSuccess: () => {
  ...
  const deleteMutation = useMutation({
    mutationFn: (id) => axios.delete(`http://localhost:8080/api/products/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries(['products']);
